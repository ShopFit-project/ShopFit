...
unsigned int nRetarget = 20; // 20 Blocks
unsigned int nStakeMinAge = 1 * 60 * 60; // 1 Hour
unsigned int nStakeMaxAge = -1;           // Unlimited
unsigned int nModifierInterval = 10 * 60; // Time to elapse before new modifier is computed
static const int64_t nTargetTimespan_legacy = nTargetSpacing * nRetarget; // 20 blocks
static const int64_t nInterval = nTargetTimespan_legacy / nTargetSpacing; // 20 Blocks

static const int64_t nTargetTimespan = 16 * 60;

int64_t devCoin = 0 * COIN;
int nCoinbaseMaturity = 5; //Amount of blocks for coins to mature
...
const string strMessageMagic = "ShopFit Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // ShopFit: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // ShopFit: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
    ...
    if (fCheckInputs)
    {
        ...
        int64_t txMinFee = tx.GetMinFee(100, GMF_RELAY, nSize);
        if (nFees < txMinFee)
...
const int DAILY_BLOCKCOUNT =  1500;
...
// ShopFit: find last block index up to pindex
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // ShopFit: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // ShopFit: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // ShopFit: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // ShopFit: track money supply and mint amount info
...
// ShopFit: total coin age spent in transaction, in the unit of coin-days.
...
// ShopFit: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // ShopFit: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // ShopFit: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // ShopFit: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // ShopFit: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    // ShopFit: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // ShopFit: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // ShopFit: getblocks may not obtain the ancestor block rejected
    ...
    // ShopFit: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "ShopFit", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "ShopFit criptocurrency created by Gabi, The Blockchain Budy for shopfit.ro";
        CTransaction txNew;
        txNew.nTime = 1565888148;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x8d89b57c92d683e59d563032063694bb9d10d280478535284a596be21a2cd856"));
        block.print();
        ...
        // ShopFit: initialize synchronized checkpoint
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // ShopFit: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // ShopFit: tell downloading node about the latest block if it's
